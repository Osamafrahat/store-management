        ...
        <thead>
          <tr className="border-b border-gray-200 dark:border-gray-700">
            ...
            {days.map(d => {
              ...
              return (
                <th key={d.day} className={`text-center px-2 py-3 font-medium text-xs min-w-[44px] ${isToday ? 'text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/10' : isWeekend ? 'text-gray-400 dark:text-gray-500' : 'text-gray-600 dark:text-gray-300'}`}>
                  <div>{d.day}</div>
                  <div className="text-[10px] font-normal">{t(`days.${dayKeys[d.dayOfWeek]}`) || dayHeaders[d.dayOfWeek]}</div>
                </th>
              )
            })}
          </tr>
              ...
              {days.map(d => {
                ...
                return (
                  <td
                    ...
                    className={`text-center px-1 py-1.5 ${record ? 'cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700' : ''} ${isWeekend ? 'bg-gray-50 dark:bg-gray-750' : ''}`}
                  >
                    {record ? (
                      <div className={`inline-flex items-center justify-center w-7 h-7 rounded-lg text-xs font-medium ${STATUS_COLORS[record.status] || ''}`} title={`${t(STATUS_KEYS[record.status]) || record.status}${record.clock_in ? ` | ${new Date(record.clock_in).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : ''}${record.clock_out ? ` → ${new Date(record.clock_out).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : ''}`}>
